...
class DetectTrack:
    ...
    def __init__(self, mode):
        ...
        self.CLASS_ID = [i for i in range(0,79)]


    def detect_track(self, frame, itr, save=False, show=True):
        print('entered detect_track')
        results = self.model(frame)
        self.frame = frame
        print(results[0].boxes, len(results))
        ...
        mask = np.array(
                    [class_id in self.CLASS_ID for class_id in detections.class_id], dtype=bool)
        ...
        if save:
            cv2.imwrite(f'output/{self.mode}/{itr}.jpg',self.frame)
        if show:
            self.frame = cv2.resize(self.frame, (960, 720), interpolation = cv2.INTER_AREA)
